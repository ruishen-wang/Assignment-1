...
public class ParticipantInfo
{
	...
	{
		ArrayList<Athletes> athletesWithResult = new ArrayList<Athletes>();
		for (int i = 0; i < athletesList.length; i++)
		{
			athletesWithResult.add(athletesList[i]);
		}
		return athletesWithResult;
	}
	...
	public String getNameThroughID(String id){
		for (int i = 0; i < this.athletesList.length; i++)
		{
			if (this.athletesList[i].equals(id))
			{
				return this.athletesList[i].getName();
